...
> **这不是一个"一键生成口播视频"的工具。**
> 它把一条口播视频拆成五个环节，**你亲自参与每一个环节**：AI 负责收集资料、起草、合成、剪辑，而**选题的角度、资料的去留、稿件的取舍、音色与形象的审美、每一段的最终确认——都由你决定**。AI 是助手，你是导演。用自己的审美和判断决定 AI 的走向，来辅助自己完成创作。

流程：**①选题 → ②深度调研 → ③统一成稿 → ④分段合成 TTS → ⑤数字人视频 + 自动剪辑**

每个环节都有明确的"确认"动作（如「资料我已审核，传递给下一个环节使用」），确认权始终在你手里。
...
- **五阶段流水线，人在每个环节拍板**：灵感收集 → 联网调研取证 → 统一成稿 → 30~40 字语义分段合成 → 数字人口播视频 → 自动粗剪 + 精剪；①②的资料须你审核确认才会传入下一环节
- **云端 TTS**：无需本地模型；支持自定义音色克隆（上传参考音频 + 对应文本），不传则用内置默认音色
- **数字人工作台**：形象图库拖拽绑定、一键批量生成、文本直生成 TTS、任务状态实时反馈；**自动排队**——RunningHub 不支持多线并行时，超出并发上限的任务自动排队，前一个完成自动续交（421 并发限制下会等待重试，不会误标失败）
- **HyperFrames 精剪**：40+ 已适配包装组件（标题 / 字幕 / 氛围 / 信息条 / 转场 / 片尾 / 数据 / 对比 / 清单），支持 AI 选配、版本回退、按反馈重剪
- **🎁 彩蛋**：任务运行时页面留白处会蹲一只 Live2D 黑猫 —— 点它看看作者是谁 ☕；流程没走对（还没建项目 / 没填 API Key）时，它也会冒出来用气泡一步步提醒你

## 🚀 快速开始

### 方式一：下载完整包（推荐，零依赖）

到本仓库的 **[Releases](../../releases)** 下载完整包 zip，解压后双击 `koubo-studio-cloud.exe` 即可（已内置 FFmpeg、Node.js 与全部运行库，无需安装任何环境）。
...
python -m uvicorn app:app --host 127.0.0.1 --port 8795
```
